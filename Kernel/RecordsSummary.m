...
DataRulesForClassifyQ[data_] :=
    MatchQ[data, {Rule[_?AtomQ, _] ..}] || DataArrayRulesForClassifyQ[data];

Clear[DataArrayRulesForClassifyQ];
DataArrayRulesForClassifyQ[data_] :=
    MatchQ[data, {Rule[_List, _] ..}] && ArrayQ[data[[All, 1]]];

Clear[NumericVectorSummary];
...
(*The option Thread\[Rule]False is just for compatibility with RecordsSummary.*)
...
DataColumnsSummary[dataColumns_, columnNamesArg_, opts : OptionsPattern[]] :=
    Block[{columnTypes, columnNames = columnNamesArg,
      ...
      If[numberedColumnsQ,
        columnNames =
            MapIndexed[ToString[#2[[1]]] <> " " <> ToString[#1] &, columnNames]
      ];
      ...
      MapThread[
        Column[{
          ...
          Switch[#2,
            ...
            Symbol,
            Grid[CategoricalVectorSummary[#3, maxTallies], Alignment -> Left]
          ]}] &,
...
RecordsSummary::args =
    "The first argument is expected to be a full array of depth 1 or 2, a \
dataset that can be converted to such a full array, an association, or a list \
of rules.";
RecordsSummary::igncols =
    "When the first argument is a dataset the second, column names argument is \
ignored.";

Clear[RecordsSummary];

(*SyntaxInformation[RecordsSummary]={"ArgumentsPattern"\[Rule]{__,\
OptionsPattern[]}};*)

Options[RecordsSummary] = Options[DataColumnsSummary];
...
RecordsSummary[dataRecords_Dataset, opts : OptionsPattern[]] :=
    Block[{row1, colKeys, records},
      ...
      If[MatchQ[row1, _Association], colKeys = Keys[row1];,
...
RecordsSummary[dataRecords_?DataRulesForClassifyQ, varNames_Rule,
  opts : OptionsPattern[]] :=
    Block[{newArgs = {opts}},
      newArgs = DeleteCases[newArgs, Rule[Thread, __]];
      Rule @@
          MapThread[
            RecordsSummary[#1, #2, newArgs] &, {Transpose[List @@@ dataRecords],
            Map[Flatten@*List, List @@ varNames]}]
...
RecordsSummary[dataRecords_?DataRulesForClassifyQ, args___] :=
    Block[{newArgs = {args}},
      newArgs = DeleteCases[newArgs, Rule[Thread, __]];
      Rule @@
          Map[RecordsSummary[#, newArgs] &, Transpose[List @@@ dataRecords]] /;
          MemberQ[{args}, Thread -> True]
    ];
